		...
		<form action="ChangeUser.action?ids=<s:property value="ids"/>"
			...
			<table width="80%" border="1" height="500" class="table">
				...
				<tr>
					<td height="40">QQ号</td>
					<td><input type="number"
						value='<s:property  value="user.userQq" />' name="user.userQq"
						class="layui-input"></td>
				</tr>
				<tr>
					<td height="40">所属医院</td>
					<td><s:if test="user.docBelongHos!=null">
							<input type="text"
								value='<s:property value=" user.docBelongHos" />'
								name="user.docBelongHos" class="layui-input">
						</s:if> <s:else>
							<input type="text" value="暂无信息" class="layui-input"
								name="user.docBelongHos">
						</s:else></td>
				</tr>
				<tr>
					<td height="40">所在部门</td>
					<td><s:if test="user.docBelongPart != null">
							<input type="text"
								value='<s:property  value=" user.docBelongPart"  />'
								name="user.docBelongPart" class="layui-input">
						</s:if> <s:else>
							<input type="text" value="暂无信息" name="user.docBelongPart"
								class="layui-input">
						</s:else></td>
				</tr>
				<tr>
					<td height="40">所在科室</td>
					<td><s:if test="department.name !=null">
							<input type="text"
								value='<s:property value=" department.name"  />'
								name="department.name" class="layui-input">
						</s:if> <s:else>
							<input type="text" value="暂无信息" name="department.name"
								class="layui-input">
						</s:else></td>
				</tr>
				<tr>
					<td height="40">目前职称</td>
					<td><s:if test="user.docTitle!=null">
							<input type="text" value='<s:property value=" user.docTitle" />'
								name="user.docTitle" class="layui-input">
						</s:if> <s:else>
							<input type="text" value="暂无信息" name="user.docTitle"
								class="layui-input">
						</s:else></td>
				</tr>
				<tr>
